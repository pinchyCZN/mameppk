...
sub MakeSrc
{
	...
	open (MAME_ADD, ">$textdir/tp_add_mame.txt") || die "$!";
	open (WINDOWS_ADD, ">$textdir/tp_add_windows.txt") || die "$!";
	open (UI_ADD, ">$textdir/tp_add_ui.txt") || die "$!";
	...
	foreach (sort keys %TEMPLATE)
	{
		...
		if ($ui ne '')
		{
			if ($TEMPLATE{$text} =~ /tp_add_ui.txt /)
			{
				print UI_ADD "# $ui\n$text\n\n\n";
				undef $ui;
			}
			elsif ($TEMPLATE{$text} =~ /tp_ui.txt /)
			{
				print UI "# $ui\n$text\n\n\n";
				undef $ui;
			}
		...
		if ($windows ne '')
		{
			if ($TEMPLATE{$text} =~ /tp_add_windows.txt /)
			{
				print WINDOWS_ADD "# $windows\n$text\n\n\n";
				undef $windows;
			}
			elsif ($TEMPLATE{$text} =~ /tp_windows.txt /)
			{
				print WINDOWS "# $windows\n$text\n\n\n";
				undef $windows;
			}
		}
		elsif ($ref !~ /^\s+$/ &&  $TEMPLATE{$text} =~ /tp_add_mame.txt /)
		{
			print MAME_ADD "#$ref\n$text\n\n\n";
		}
		unless ($ref =~ /^\s+$/)
