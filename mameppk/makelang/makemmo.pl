...
sub MakeListFiles
{
	...
	my $filename_add = "mame_add_$lang.lst";
	...
	unless (-f "$textdir/$filename_add")
	{
		print "Not found: $filename_add\n";
		return;
	}
	...
	print "Converting $filename_add...\n";

	open (IN, "$textdir/$filename_add") || die "$!";
	while (<IN>)
	{
		s/[\r\n]*$//;
...
sub MakeMoFiles
{
	...
	foreach (sort @dirs_add)
	{
		next unless /^$lang[_]add[_](.*)\.txt$/;
		my $target = $1;
		my $textfile = $_;
		&ReadTemplateFiles("add_$target");
		&MakeMo($textfile, $target);
	}
	close (NOTFOUND);
}
...
sub ReadTemplateFiles
{
	...
	if (0 && ($target eq "ui") || ($target eq "mame"))
	{
		$file = "tp_add_$target.txt";
		print "$file\n";
