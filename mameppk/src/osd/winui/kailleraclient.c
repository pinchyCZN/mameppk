...
static TCHAR DLL_Name[MAX_PATH];
...
int kailleraChatSendUTF8(char *s)
{
	char *stemp;
	int ret = 0;

	stemp = astring_from_utf8(s);
	if (stemp)
	{
...
BOOL LoadLibrary_KailleraClient_DLL(const TCHAR *fname)
{
	...
	kailleraGetVersion = (int (WINAPI *) (char *))GetProcAddress((HINSTANCE)hDLL, "_kailleraGetVersion@4");
	if (kailleraGetVersion == NULL)	goto err;
	kailleraInit = (int (WINAPI *)(void))GetProcAddress((HINSTANCE)hDLL, "_kailleraInit@0");
	if (kailleraInit == NULL)	goto err;
	kailleraShutdown = (int (WINAPI *)(void))GetProcAddress((HINSTANCE)hDLL, "_kailleraShutdown@0");
	...
	kailleraChatSend = (int (WINAPI *)(char *))GetProcAddress((HINSTANCE)hDLL, "_kailleraChatSend@4");
	if (kailleraChatSend == NULL)	goto err;

	kailleraEndGame = (int (WINAPI *)(void))GetProcAddress((HINSTANCE)hDLL, "_kailleraEndGame@0");
