	...
	case WM_INITDIALOG:
	{
		...
		if (folder != NULL)
		{
			...
			if ( GetFilterInherit() )
			{
				...
				if( ! bShowExplanation )
				{
					EnableWindow(GetDlgItem(hDlg, IDC_INHERITED), FALSE );
				}
			}
			else
			{
...
INT_PTR CALLBACK DirectXDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	const WCHAR *directx_help =
		TEXT(MAMEUINAME)
		TEXT(" requires DirectX version 3 or later, which is a set of operating\r\n")
		...
		TEXT("download DirectX, install it, and then run ")
		TEXT(MAMEUINAME)
		TEXT(" again.\r\n");
