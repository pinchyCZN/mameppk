...
base *drawd3d9_init(void)
{
	...
	if (dllhandle == NULL)
	{
		osd_printf_verbose(_WINDOWS("Direct3D: Unable to access d3d9.dll\n"));
		return NULL;
	}
	...
	{
		osd_printf_verbose(_WINDOWS("Direct3D: Unable to find Direct3DCreate9\n"));
	...
	if (d3d9 == NULL)
	{
		osd_printf_verbose(_WINDOWS("Direct3D: Error attempting to initialize Direct3D9\n"));
		FreeLibrary(dllhandle);
		dllhandle = NULL;
		return NULL;
	}
	...
	if (fxhandle == NULL)
	{
		osd_printf_verbose(_WINDOWS("Direct3D: Warning - Unable find any D3D9 DLLs; disabling post-effect rendering\n"));
	...
	osd_printf_verbose(_WINDOWS("Direct3D: Using Direct3D 9\n"));
	return d3dptr;
}
