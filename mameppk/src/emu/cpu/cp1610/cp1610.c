 ...
 *  illegal opcodes
 ***********************************/
void cp1610_cpu_device::cp1610_illegal()
{
 ...
 *  - - - -  HLT
 ***************************************************/
void cp1610_cpu_device::cp1610_hlt()
{
	/* TBD */
	m_icount -= 4;
}
 ...
 *  x x - -  INCR  Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_incr(int n)
{
	m_r[n]++;
	CLR_SZ;
	SET_SZ(m_r[n]);
 ...
 *  x x - -  DECR  Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_decr(int n)
{
	m_r[n]--;
	CLR_SZ;
	SET_SZ(m_r[n]);
 ...
 *  x x - -  COMR  Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_comr(int n)
{
	m_r[n] ^= 0xffff;
	CLR_SZ;
	SET_SZ(m_r[n]);
 ...
 *  - - - -  GSWD  Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_gswd(int n)
{
	m_r[n] = (m_flags << 8) + m_flags;
	m_icount -= 6;
}
 ...
 *  - - - -  NOP
 ***************************************************/
void cp1610_cpu_device::cp1610_nop()
{
	m_icount -= 6;
}
 ...
 *  - - - -  SIN
 ***************************************************/
void cp1610_cpu_device::cp1610_sin()
{
	/* TBD */
	m_icount -= 6;
}
 ...
 *  x x x x  RSWD Rs
 ***************************************************/
void cp1610_cpu_device::cp1610_rswd(int n)
{
	m_flags = m_r[n];
	m_icount -= 6;
}
 ...
 *  x x - -  TSTR Rs
 ***************************************************/
void cp1610_cpu_device::cp1610_tstr(int n)
{
	CLR_SZ;
	SET_SZ(m_r[n]);
	m_icount -= 6;
 ...
 *  x x - -  MOVR Rs,Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_movr(int s, int d)
{
	CLR_SZ;
	m_r[d] = m_r[s];
	SET_SZ(m_r[d]);
 ...
 *  x x x x  ADDR Rs, Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_addr(int s, int d)
{
	CLR_SZOC;
	SET_COV(m_r[s],m_r[d],0);
	m_r[d] += m_r[s];
	SET_SZ(m_r[d]);
	m_icount -= 6;
}
 ...
 *  x x - -  ANDR Rs, Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_andr(int s, int d)
{
	CLR_SZ;
	m_r[d] &= m_r[s];
	SET_SZ(m_r[d]);
 ...
 *  x x - -  XORR Rs, Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_xorr(int s, int d)
{
	CLR_SZ;
	m_r[d] ^= m_r[s];
	SET_SZ(m_r[d]);
 ...
 *  x x - -  CLRR Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_clrr(int d)
{
	CLR_SZ;
	m_r[d] = 0;
	SET_SZ(m_r[d]);
 ...
 *  - - - -  NOPP
 ***************************************************/
void cp1610_cpu_device::cp1610_nopp(int dir)
{
	cp1610_readop(m_r[7]);
	m_r[7]++;
	m_icount -= 7;
 ...
 *  - - - -  BLT ADDR
 ***************************************************/
void cp1610_cpu_device::cp1610_blt(int dir)
{
	int condition1 = 0;
	int condition2 = 0;
	UINT16 offset = cp1610_readop(m_r[7]);
 ...
 *  - - - -  BGE ADDR
 ***************************************************/
void cp1610_cpu_device::cp1610_bge(int dir)
{
	int condition1 = 0;
	int condition2 = 0;
	UINT16 offset = cp1610_readop(m_r[7]);
 ...
 *  - - - -  BLE ADDR
 ***************************************************/
void cp1610_cpu_device::cp1610_ble(int dir)
{
	int condition1 = 0;
	int condition2 = 0;
	UINT16 offset = cp1610_readop(m_r[7]);
	...
	if (m_flags & S) condition1 = 1;
	if (m_flags & OV) condition2 = 1;
	if ((m_flags & Z) || (condition1 ^ condition2))
	{
		m_r[7] += (offset ^ dir);
		m_icount -= 9;
	}
 ...
 *  - - - -  BGT ADDR
 ***************************************************/
void cp1610_cpu_device::cp1610_bgt(int dir)
{
	int condition1 = 0;
	int condition2 = 0;
	UINT16 offset = cp1610_readop(m_r[7]);
 ...
 *  - - - -  BUSC ADDR
 ***************************************************/
void cp1610_cpu_device::cp1610_busc(int dir)
{
	int condition1 = 0;
	int condition2 = 0;
	UINT16 offset = cp1610_readop(m_r[7]);
 ...
 *  - - - -  BESC ADDR
 ***************************************************/
void cp1610_cpu_device::cp1610_besc(int dir)
{
	int condition1 = 0;
	int condition2 = 0;
	UINT16 offset = cp1610_readop(m_r[7]);
 ...
 *  - - - -  MVI@ Rm, Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_mviat(int m, int d)
{
 ...
 *  - - - -  PULR Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_pulr(int d)
{
	m_r[6]--;
	m_r[d] = cp1610_readmem16(m_r[6]);
	m_icount -= 11;
}
 ...
 *  - - - -  SDBD, MVI@ Rm, Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_sdbd_mviat(int r, int d)
{
	m_r[d] = cp1610_readmem16(m_r[r]) & 0xff;
	m_r[d] |= (cp1610_readmem16(m_r[r]) << 8);
	m_icount -= 14;
}
 ...
 *  - - - -  SDBD, MVI@ Rm, Rd
 ***************************************************/
void cp1610_cpu_device::cp1610_sdbd_mviat_d(int r, int d)
{
	m_r[r]--;
	m_r[d] = cp1610_readmem16(m_r[r]) & 0xff;
	m_r[r]--;
...
void cp1610_cpu_device::execute_run()
{
	...
	do
	{
		...
		m_r[7]++;
#if 0
		logerror("PC:0x%04x, opcode = 0x%03x, ",m_r[7]-1,opcode);
		logerror("R0:0x%04x, ",m_r[0]);
		logerror("R1:0x%04x, ",m_r[1]);
		logerror("R2:0x%04x, ",m_r[2]);
		logerror("R3:0x%04x, ",m_r[3]);
		logerror("R4:0x%04x, ",m_r[4]);
		logerror("R5:0x%04x, ",m_r[5]);
		logerror("R6:0x%04x\n",m_r[6]);
#endif
...
void cp1610_cpu_device::device_start()
{
	...
	save_item(NAME(m_intr_pending));
	save_item(NAME(m_intrm_pending));
	save_item(NAME(m_mask_interrupts));

	// Register state for debugger
	state_add( CP1610_R0, "R0", m_r[0] ).formatstr("%04X");
	state_add( CP1610_R1, "R1", m_r[1] ).formatstr("%04X");
	state_add( CP1610_R2, "R2", m_r[2] ).formatstr("%04X");
	state_add( CP1610_R3, "R3", m_r[3] ).formatstr("%04X");
	state_add( CP1610_R4, "R4", m_r[4] ).formatstr("%04X");
