	...
	for (audit_record *record = m_record_list.first(); record != NULL; record = record->next())
	{
		...
		if (output != NULL)
		{
			...
			if (record->expected_length() > 0)
				strcatprintf(*output,_(" (%" I64FMT "d bytes)"), record->expected_length());
			strcatprintf(*output," - ");
		}
		...
		switch (record->substatus())
		{
			case audit_record::SUBSTATUS_GOOD_NEEDS_REDUMP:
				if (output != NULL) strcatprintf(*output,_("NEEDS REDUMP\n"));
				best_new_status = BEST_AVAILABLE;
				break;

			case audit_record::SUBSTATUS_FOUND_NODUMP:
				if (output != NULL) strcatprintf(*output,_("NO GOOD DUMP KNOWN\n"));
				best_new_status = BEST_AVAILABLE;
				break;
			...
			case audit_record::SUBSTATUS_FOUND_BAD_CHECKSUM:
				...
				{
					std::string tempstr;
					strcatprintf(*output,_("INCORRECT CHECKSUM:\n"));
					strcatprintf(*output,_("EXPECTED: %s\n"), record->expected_hashes().macro_string(tempstr));
					strcatprintf(*output,_("   FOUND: %s\n"), record->actual_hashes().macro_string(tempstr));
				}
				break;

			case audit_record::SUBSTATUS_FOUND_WRONG_LENGTH:
				if (output != NULL) strcatprintf(*output,_("INCORRECT LENGTH: %" I64FMT "d bytes\n"), record->actual_length());
				break;
			...
			case audit_record::SUBSTATUS_NOT_FOUND:
				if (output != NULL)
				{
					...
					if (shared_device == NULL)
						strcatprintf(*output,_("NOT FOUND\n"));
					else
						strcatprintf(*output,_("NOT FOUND (%s)\n"), shared_device->shortname());
				}
				best_new_status = NOTFOUND;
				break;

			case audit_record::SUBSTATUS_NOT_FOUND_NODUMP:
				if (output != NULL) strcatprintf(*output,_("NOT FOUND - NO GOOD DUMP KNOWN\n"));
				best_new_status = BEST_AVAILABLE;
				break;

			case audit_record::SUBSTATUS_NOT_FOUND_OPTIONAL:
				if (output != NULL) strcatprintf(*output,_("NOT FOUND BUT OPTIONAL\n"));
				best_new_status = BEST_AVAILABLE;
				break;
