...
#ifdef KAILLERA
		IPT_UI_KAILLERA_PLAYER_INC, /* kt */
		IPT_UI_KAILLERA_PLAYER_DEC,
		IPT_UI_KAILLERA_PLAYER_2IN,
		IPT_UI_KAILLERA_PLAYER_POS,
		IPT_UI_KAILLERA_PLAYER_DUP,
		IPT_UI_KAILLERA_TEST_1,
		IPT_UI_KAILLERA_TEST_2,
		IPT_UI_KAILLERA_TEST_3,
		IPT_UI_KAILLERA_TEST_4,
		IPT_UI_KAILLERA_TEST_5,
		IPT_UI_KAILLERA_TEST_6,
		IPT_UI_KAILLERA_TEST_7,
		IPT_UI_KAILLERA_TEST_8,
		IPT_UI_KAILLERA_TEST_9,
		IPT_UI_KAILLERA_TEST1_0,
		IPT_UI_KAILLERA_TEST1_1,
		IPT_UI_KAILLERA_TEST1_2,
		IPT_UI_KAILLERA_TEST1_3,
		IPT_UI_KAILLERA_TEST2_4,
		IPT_UI_KAILLERA_TEST2_5,
		IPT_UI_KAILLERA_TEST2_6,
		IPT_UI_KAILLERA_TEST2_7,
		IPT_UI_KAILLERA_TEST2_0,
		IPT_UI_KAILLERA_TEST2_1,
		IPT_UI_KAILLERA_TEST2_2,
		IPT_UI_KAILLERA_TEST2_3,
		IPT_UI_KAILLERA_TEST1_4,
		IPT_UI_KAILLERA_TEST1_5,
		IPT_UI_KAILLERA_TEST1_6,
		IPT_UI_KAILLERA_TEST1_7,
//		IPT_UI_TOGGLE_LOGWAVE,
		IPT_UI_KAILLERA_TEST1_9,
	
		IPT_UI_CHAT_OPEN,
		IPT_UI_CHAT_CHANGE_MODE,
		IPT_UI_CHAT_SHOW_LOG,
#endif /* KAILLERA */
...
#ifdef KAILLERA
struct _inpsub_header
{
	char str[12];		 /* 'EMEMAMETRACE' */
	long trcversion;	 /* trcファイルのバージョン */
	char name[256];      /* game->name */
	char version[256];   /* mame version */

	char usestate[256];  /* 使用するステートセーブ */
	unsigned char playcount;	/* MAME32を起動してから、プレイしたゲーム回数 */
};
typedef struct _inpsub_header inpsub_header;

#define INPUTLOG_TRACE_VERSION 1
#endif /* KAILLERA */
...
#ifdef USE_CUSTOM_BUTTON
	void append_custom(device_t &device, astring &errorbuf);
#endif /* USE_CUSTOM_BUTTON */
};
...
public:
	...
	bool has_record_file() { return m_record_file.is_open(); };
	bool has_playback_file() { return m_playback_file.is_open(); };
#ifdef INP_CAPTION
	bool has_caption_file() { return m_caption_file.is_open(); };
	void draw_caption(render_container *container);
#endif /* INP_CAPTION */
#ifdef KAILLERA
	bool has_record_sub_file() { return m_record_sub_file.is_open(); };
	bool has_playback_sub_file() { return m_playback_sub_file.is_open(); };
//	emu_file &record_sub_file() { return m_record_sub_file; };
...
#ifdef USE_SHOW_INPUT_LOG
	void copy_command_buffer(char log);
	void make_input_log();
#endif /* USE_SHOW_INPUT_LOG */

#ifdef KAILLERA
	void input_port_value_save();
	void input_port_value_load();
	int readword(emu_file &f, UINT16 *num);
	void writeword(emu_file &f, UINT16 num);
#endif /* KAILLERA */
...
#ifdef INP_CAPTION
	emu_file                m_caption_file;	        // caption file for playback (NULL if not playing)
	int                     m_next_caption_frame;
	int                     m_caption_timer;
#endif /* INP_CAPTION */
#ifdef KAILLERA
	emu_file                m_record_sub_file;      // recording file (NULL if not recording)
	emu_file                m_playback_sub_file;    // playback file (NULL if not recording)
#endif /* KAILLERA */
