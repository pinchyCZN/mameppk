...
static emu_options *lang_options;
...
int lang_find_langname(const char *name)
{
	...
	{
		if (!core_stricmp(ui_lang_info[i].name, name))
			return i;
		if (!core_stricmp(ui_lang_info[i].shortname, name))
			return i;
	}
...
void lang_set_langcode(emu_options &options, int langcode)
{
