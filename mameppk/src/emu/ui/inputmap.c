...
void ui_menu_input::populate_and_sort(input_item_data *itemlist)
{
	...
	for (curitem = 0; curitem < numitems; curitem++)
	{
		...
		if (item->owner_name && strcmp(item->owner_name, prev_owner.c_str()) != 0)
		{
			...
			item_append(_(text.c_str()), NULL, 0, NULL);
			prev_owner.assign(item->owner_name);
		}

		strprintf(text, nameformat[item->type], _(item->name));
		...
		/* add the item */
		item_append(_OSD0(_(text.c_str())), _OSD0(_(subtext.c_str())), flags, item);
	}
}
		...
		for (field = port->first_field(); field != NULL; field = field->next())
			if (field->type() == type && field->enabled())
			{
				...
				if (strcmp(field->device().tag(), prev_owner.c_str()) != 0)
				{
					...
					item_append(_(name.c_str()), NULL, 0, NULL);
					prev_owner.assign(field->device().tag());
				}

				name.assign(field->name());

				item_append(_(name.c_str()), _(field->setting_name()), flags, (void *)field);
					...
					if (type != ANALOG_ITEM_CENTERSPEED || use_autocenter)
					{
						...
						switch (type)
						{
							...
							case ANALOG_ITEM_KEYSPEED:
								strprintf(text, _("%s Digital Speed"), name.c_str());
							...
							case ANALOG_ITEM_CENTERSPEED:
								strprintf(text, _("%s Autocenter Speed"), name.c_str());
							...
							case ANALOG_ITEM_REVERSE:
								strprintf(text, _("%s Reverse"), name.c_str());
							...
							case ANALOG_ITEM_SENSITIVITY:
								strprintf(text, _("%s Sensitivity"), name.c_str());
						...
						/* append a menu item */
						item_append(_(text.c_str()), _(subtext.c_str()), flags, data);
					}
			}
}
