...
void ui_menu_input::populate_and_sort(input_item_data *itemlist)
{
	...
	for (curitem = 0; curitem < numitems; curitem++)
	{
		...
		if (item->owner_name && strcmp(item->owner_name, prev_owner.cstr()) != 0)
		{
			...
			item_append(_(text), NULL, 0, NULL);
		...
		/* add the item */
		item_append(_OSD0(_(text)), _OSD0(_(subtext)), flags, item);
	}
}
		...
		for (field = port->first_field(); field != NULL; field = field->next())
			if (field->type() == type && field->enabled())
			{
				...
				if (strcmp(field->device().tag(), prev_owner.cstr()) != 0)
				{
					...
					item_append(_(name), NULL, 0, NULL);
					prev_owner.cpy(field->device().tag());
				}

				name.cpy(field->name());

				item_append(_(name), _(field->setting_name()), flags, (void *)field);
					...
					if (type != ANALOG_ITEM_CENTERSPEED || use_autocenter)
					{
						...
						switch (type)
						{
							...
							case ANALOG_ITEM_KEYSPEED:
								text.printf(_("%s Digital Speed"), name.cstr());
							...
							case ANALOG_ITEM_CENTERSPEED:
								text.printf(_("%s Autocenter Speed"), name.cstr());
							...
							case ANALOG_ITEM_REVERSE:
								text.printf(_("%s Reverse"), name.cstr());
							...
							case ANALOG_ITEM_SENSITIVITY:
								text.printf(_("%s Sensitivity"), name.cstr());
						...
						/* append a menu item */
						item_append(_(text), _(subtext), flags, data);
					}
			}
}
