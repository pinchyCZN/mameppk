...
void ui_menu_image_info::image_info(device_image_interface *image)
{
	if (image->exists())
	{
		...
		if (image->software_entry())
		{
			...
			switch (image->supported())
			{
				case SOFTWARE_SUPPORTED_NO:
					item_append(_("Not supported"), "", MENU_FLAG_DISABLE, NULL);
					break;
				case SOFTWARE_SUPPORTED_PARTIAL:
					item_append(_("Partially supported"), "", MENU_FLAG_DISABLE, NULL);
					break;
				default:
					break;
			}
		...
		item_append(image->brief_instance_name(), _("[empty]"), 0, NULL);
	item_append("", NULL, MENU_FLAG_DISABLE, NULL);
}
