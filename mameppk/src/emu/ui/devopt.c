...
void ui_menu_device_config::populate()
{
	...
	string.printf(_("[This option is%s currently mounted in the running system]\n\n"), m_mounted ? "" : " NOT");
	string.catprintf(_("Option: %s\n"), m_option->name());

	dev = const_cast<machine_config &>(machine().config()).device_add(&machine().config().root_device(), m_option->name(), m_option->devtype(), 0);
		
	string.catprintf(_("Device: %s\n"), dev->name());
	if (!m_mounted)
	...
	if (scriter.count() > 0)
	{
		...
		{
			string.catprintf(_("  Screen '%s': "), screen->tag());
		...
		for (ioport_field *field = port->first_field(); field != NULL; field = field->next())
		{
			...
			else if (field->type() == IPT_CONFIG)
			{
				...
				for (ioport_setting *setting = field->first_setting(); setting != NULL; setting = setting->next())
				{
					if (setting->value() == field->defvalue())
					{
						confs_opt.catprintf(_("    [default: %s]\n"), setting->name());
						break;
					}
				}
			}
	...
	if (dips)
		string.cat(_("* Dispwitch settings:\n")).cat(dips_opt);
	if (confs)
		string.cat(_("* Configuration settings:\n")).cat(confs_opt);
	if (input + input_mj + input_hana + input_gamble + input_analog + input_adjust)
		string.cat(_("* Input device(s):\n"));
	if (input)
		string.catprintf(_("  Player inputs    [%d inputs]\n"), input);
	if (input_mj)
		string.catprintf(_("  Mahjong inputs    [%d inputs]\n"), input_mj);
	if (input_hana)
		string.catprintf(_("  Hanafuda inputs    [%d inputs]\n"), input_hana);
	if (input_gamble)
		string.catprintf(_("  Gambling inputs    [%d inputs]\n"), input_gamble);
	if (input_analog)
		string.catprintf(_("  Analog inputs    [%d inputs]\n"), input_analog);
	if (input_adjust)
		string.catprintf(_("  Adjuster inputs    [%d inputs]\n"), input_adjust);

	image_interface_iterator imgiter(*dev);
	if (imgiter.count() > 0)
	{
		string.cat(_("* Media Options:\n"));
		for (const device_image_interface *imagedev = imgiter.first(); imagedev != NULL; imagedev = imgiter.next())
			string.catprintf(_("  %s    [tag: %s]\n"), imagedev->image_type_name(), imagedev->device().tag());
	}
	
	slot_interface_iterator slotiter(*dev);
	if (slotiter.count() > 0)
	{
		string.cat(_("* Slot Options:\n"));
		for (const device_slot_interface *slot = slotiter.first(); slot != NULL; slot = slotiter.next())
			string.catprintf(_("  %s    [default: %s]\n"), slot->device().tag(), slot->default_option() ? slot->default_option() : "----");
	}
