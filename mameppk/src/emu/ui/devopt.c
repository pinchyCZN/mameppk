...
void ui_menu_device_config::populate()
{
	...
	strprintf(str,_("[This option is%s currently mounted in the running system]\n\n"), m_mounted ? "" : " NOT");
	strcatprintf(str,_("Option: %s\n"), m_option->name());

	dev = const_cast<machine_config &>(machine().config()).device_add(&machine().config().root_device(), m_option->name(), m_option->devtype(), 0);

	strcatprintf(str,_("Device: %s\n"), dev->name());
	if (!m_mounted)
	...
	if (scriter.count() > 0)
	{
		...
		{
			strcatprintf(str,_("  Screen '%s': "), screen->tag());
		...
		for (ioport_field *field = port->first_field(); field != NULL; field = field->next())
		{
			...
			else if (field->type() == IPT_CONFIG)
			{
				...
				for (ioport_setting *setting = field->first_setting(); setting != NULL; setting = setting->next())
				{
					if (setting->value() == field->defvalue())
					{
						strcatprintf(confs_opt,_("    [default: %s]\n"), setting->name());
						break;
					}
				}
			}
	...
	if (dips)
		str.append(_("* Dispwitch settings:\n")).append(dips_opt);
	if (confs)
		str.append(_("* Configuration settings:\n")).append(confs_opt);
	if (input + input_mj + input_hana + input_gamble + input_analog + input_adjust + input_keypad + input_keyboard)
		str.append(_("* Input device(s):\n"));
	if (input)
		strcatprintf(str,_("  User inputs    [%d inputs]\n"), input);
	if (input_mj)
		strcatprintf(str,_("  Mahjong inputs    [%d inputs]\n"), input_mj);
	if (input_hana)
		strcatprintf(str,_("  Hanafuda inputs    [%d inputs]\n"), input_hana);
	if (input_gamble)
		strcatprintf(str,_("  Gambling inputs    [%d inputs]\n"), input_gamble);
	if (input_analog)
		strcatprintf(str,_("  Analog inputs    [%d inputs]\n"), input_analog);
	if (input_adjust)
