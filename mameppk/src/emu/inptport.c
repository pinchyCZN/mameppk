...
static const input_port_config *port_info[MAX_INPUT_PORTS];
...
#endif /* INP_CAPTION */
#ifdef KAILLERA
	emu_file *					record_sub_file;		/* recording file (NULL if not recording) */
	emu_file *					playback_sub_file;		/* playback file (NULL if not recording) */
#endif /* KAILLERA */
	...
	if (kPlay)
	{
		...
		for (port = machine->m_portlist.first(); port != NULL; port = port->next())
		{
			...
			for (field = port->fieldlist; field != NULL; field = field->next)
			{
				...
				if (field->player == 0) player=0;
				if (field->player == 1) player=1;
				if (field->player == 2) player=2;
				if (field->player == 3) player=3;
				...
				case IPT_DIPSWITCH:
					...
					if (!strcmp(field->name, "Service Mode") &&
						(field->flags & FIELD_FLAG_TOGGLE))
					{	//PORT_SERVICE(mask,default)
						player=0;	pos=14;	
						kBitsPlayValues[player][pos][0]=portnum;
						kBitsPlayValues[player][pos][1]=field->mask;
						if (field->defvalue)	kBitsPlayValues[player][pos][2]=IP_ACTIVE_LOW;
				...
				{
					if(mahjong!=-1) {
						kMahjongPlayValues[player][pos][0]=portnum;
						kMahjongPlayValues[player][pos][1]=field->mask;
						kBitsPlayValues[player][pos][2]=field->defvalue;
						kMaxMahjong++;

					} else if(dipswitch!=-1) {
						kDipSwitchValues[kMaxDipSwitch][0]=portnum;
						kDipSwitchValues[kMaxDipSwitch][1]=field->mask;
						kMaxDipSwitch++;

					} else if(analog!=-1) {
						if(KAnalogCtrl) {
							kAnalogPlayValues[player][pos]=portnum;
						...
						if (kBitsPlayValues[player][pos][0]==-1)	//	同じボタンが複数ある場合。
						{
							kBitsPlayValues[player][pos][0]=portnum;
							kBitsPlayValues[player][pos][1]=field->mask;
							if (field->mask != field->defvalue)
								kBitsPlayValues[player][pos][2]=field->defvalue;
							else
								kBitsPlayValues[player][pos][2]=IP_ACTIVE_LOW;
						} else
						{
							if (kBitsPlayValues[player][pos][0] == portnum)
							{
								kBitsPlayValues[player][pos][1]|=field->mask;
							} else
							{
								...
								{
									kBitsPlayValues[player][pos][0+3]=portnum;
									kBitsPlayValues[player][pos][1+3]=field->mask;
									if (field->mask != field->defvalue)
										kBitsPlayValues[player][pos][2]=field->defvalue;
									else
										kBitsPlayValues[player][pos][2]=IP_ACTIVE_LOW;
...
static int readword(emu_file *f,UINT16 *num)
{
	UINT16 res;

	if (f == NULL) return -1;

	if (f->read(&res, sizeof(res)) == sizeof(res))
...
static void writeword(emu_file *f,UINT16 num)
{
	UINT16 res = BIG_ENDIANIZE_INT16(num);

	if (f == NULL) return;

	if (f->write(&res, sizeof(res)) != sizeof(res))
	{
		f->close();
		f = NULL;
	}
...
g_profiler.start(PROFILER_INPUT);
	...
	for (port = machine->m_portlist.first(); port != NULL; port = port->next())
	{
		...
		for (field = port->fieldlist; field != NULL; field = field->next)
			if (input_condition_true(port->machine, &field->condition))
			{
				...
				if (field->type >= IPT_CUSTOM1 && field->type < IPT_CUSTOM1 + MAX_CUSTOM_BUTTONS)
				{
					if (input_seq_pressed(machine, input_field_seq(field, SEQ_TYPE_STANDARD)))
					{
	...
	if (kPlay)
	{
		...
		if (kFirstMsg)
		{
			...
			{
				popmessage(_("Play Game With Record Input Log As %s"), options_get_string(&machine->options(), "auto_record_name"));
			}
		...
		else
		{
			...
			{
			  if(kBitsPlayValues[0][i][0]!=-1)
			    if(port_info[kBitsPlayValues[0][i][0]]->state->digital & kBitsPlayValues[0][i][1]) val[0] |= b;
			  b=b<<1;
			}
			...
			for(j=1; j<KailleraPlayerOption.subplayer+1; j++)
			{
			    ...
			    for(i=0;i<16;i++)
				{
					if(kBitsPlayValues[j][i][0]!=-1)
						if(port_info[kBitsPlayValues[j][i][0]]->state->digital & kBitsPlayValues[j][i][1]) val[j] |= b;
					b=b<<1;
				}
			}
			
			//popmessage( IntToBit((int)val[0]));	//kt test
			//popmessage( "%08x,%08x,%08x", val[0],port_info[0]->state->defvalue,port_info[0]->state->digital);	//kt test
			{	//kt start
			
				kaillera_ipu_ui = 0;
				if(ui_input_pressed(machine, IPT_UI_SOFT_RESET))		kaillera_ipu_ui = 1;
				...
				if(Kaillera_StateSave_Count <= 0 && Kaillera_StateSave_TimeRemainder <= 0 && (~Kaillera_Send_Flags & 0x1) && KailleraChatdataPreparationcheck.timeremainder <= 0) {
					...
					if(KailleraStartOption.player == 1) { // クリエイトした人のみの操作可\x94\x5c
						int f=0;
						if(ui_input_pressed(machine, IPT_UI_LOAD_STATE))			kaillera_ipu_ui = 3;		// staロード
						if(ui_input_pressed(machine, IPT_UI_KAILLERA_TEST_7))	Kaillera_StateSave_SelectFile = ~Kaillera_StateSave_SelectFile; // ステートセーブ、ロードで使用する拡張子の変更
						if(ui_input_pressed(machine, IPT_UI_KAILLERA_TEST1_0))	Kaillera_Overclock_Flags = ~Kaillera_Overclock_Flags; // オーバークロック
						if(ui_input_pressed(machine, IPT_UI_SAVE_STATE)) {// staセーブ
							char tmp[256];
							kaillera_ipu_ui = 2;
						...
						if(ui_input_pressed(machine, IPT_UI_KAILLERA_TEST_4)) {// ディップスイッチの状態を送信
							//int j;
							long dat[64];
							...
							if( kMaxDipSwitch<=0 )
								...
								for(i=0;i<kMaxDipSwitch;i++) {
									(*val2) = (UINT16)(port_info[kDipSwitchValues[i][0]]->state->defvalue & kDipSwitchValues[i][1]);
									val2++;
								}
								if(i&0x1) i++;
						...
						if(ui_input_pressed(machine, IPT_UI_KAILLERA_TEST_9)) { KailleraPlayerOption.sendfilespeed = KailleraStartOption.send_file_speed; f=2; }
						...
						if(f>0) {
							...
							emu_file file = emu_file(machine->options(), SEARCHPATH_STATE, OPEN_FLAG_READ);
				...
				if(ui_input_pressed(machine, IPT_UI_KAILLERA_PLAYER_DUP))
					if(KailleraStartOption.player == 1)		kaillera_ipu_ui = 4;
				
				
				if(KailleraPlayerOption.playercontrol) {	//2 Input のプレイヤー位置移動
					int ipu_ui = 0;
					if(ui_input_pressed(machine, IPT_UI_KAILLERA_PLAYER_INC))ipu_ui = 7;
					if(ui_input_pressed(machine, IPT_UI_KAILLERA_PLAYER_DEC))ipu_ui = 6;
					...
					if(ui_input_pressed(machine, IPT_UI_KAILLERA_PLAYER_INC))kaillera_ipu_ui = 7;
					if(ui_input_pressed(machine, IPT_UI_KAILLERA_PLAYER_DEC))kaillera_ipu_ui = 6;
				}
				
				
				if(ui_input_pressed(machine, IPT_UI_KAILLERA_TEST_1) && KailleraStartOption.player == 1) {
					if( Kaillera_StateSave_Flags & KAILLERA_STATESAVE_AUTOSAVE ) {
						...
						KailleraStartOption.autosave_time_interval	= (int)(69.0 * defrefresh + 0.5);
					}
				}
				if(ui_input_pressed(machine, IPT_UI_KAILLERA_TEST_3) && KailleraStartOption.player == 1) {// mameのバージョンチェック
					long dat[64];
					dat[0] = 0x00000005;
					dat[1] = 0;
				...
				if(ui_input_pressed(machine, IPT_UI_KAILLERA_TEST_2) && KailleraStartOption.player == 1)	// 時間調節レベル変更
				{
					long dat[64];
					dat[0] = 0x0000000d;
				...
				if(ui_input_pressed(machine, IPT_UI_KAILLERA_TEST1_2) 
					...
					for(i=0;i<29;i++)
					{
						...
						if(kMahjongPlayValues[0][i][0]!=-1)
						{
							if(kMahjongPlayValues[0][i][2] != IP_ACTIVE_LOW) {
								//if( port_info[kMahjongPlayValues[0][i][0]]->state->digital & kMahjongPlayValues[0][i][1] ) {
								if(!(port_info[kMahjongPlayValues[0][i][0]]->state->digital & kMahjongPlayValues[0][i][1]) ) {
			...
			//popmessage( "%08x,%08x,%08x,%d", val[0],port_info[0].defvalue,port_info[0]->state->digital,chval[KAILLERA_VAL_LEN + (Kaillera_analog_port * KAILLERA_ANALOG_LEN)]);	//kt test
				...
				if(ui_input_pressed(machine, IPT_UI_KAILLERA_TEST1_6))
				{	//kt test
			...
			//popmessage( "%08x,%08x,%08x,%d", val[0],port_info[0]->state->defvalue,port_info[0]->state->digital,recvLen);	//kt test
			...
			if(kPlay){	//kt start
				...
				if(Kaillera_StateSave_TimeRemainder == KAILLERA_STATESAVE_NORMAL_DELAYTIME) { // セーブが完了したので、CRCをチェック。
					...
					crc = game_ram_serch_crc32_kaillera_state_save(machine, Kaillera_Inp_StateSave_CRC);
				...
				if (KailleraSyncCheck.count)	//同期チェック
				{
					if (synccount == KailleraSyncCheck.pos)
					{
						...
						KailleraSyncCheck.crc[KailleraStartOption.player-1][KailleraSyncCheck.totalcount - KailleraSyncCheck.count] = game_ram_serch_crc32_(machine, 0);	//ゲームCPUが使用しているメモリのCRCを使う
					...
					for(j=0;j<16;j++)
					{
						...
						if(kBitsPlayValues[i][j][0]!=-1)
						{
							if(kBitsPlayValues[i][j][2] != IP_ACTIVE_LOW) {
								//if(!(val[i] & b))
								if(val[i] & b)
									port_info[kBitsPlayValues[i][j][0]]->state->digital |= kBitsPlayValues[i][j][1];
								else
									port_info[kBitsPlayValues[i][j][0]]->state->digital &= ~kBitsPlayValues[i][j][1]; //kt
							} else {
								//if(val[i] & b)
								if(!(val[i] & b))
									port_info[kBitsPlayValues[i][j][0]]->state->digital &= ~kBitsPlayValues[i][j][1];
						...
						if(kBitsPlayValues[i][j][0+3]!=-1)
						{
							if(kBitsPlayValues[i][j][2+3] != IP_ACTIVE_LOW) {
								//if(!(val[i] & b))
								if(val[i] & b)
									port_info[kBitsPlayValues[i][j][0+3]]->state->digital |= kBitsPlayValues[i][j][1+3];
								else
									port_info[kBitsPlayValues[i][j][0+3]]->state->digital &= ~kBitsPlayValues[i][j][1+3]; //kt
							} else {
								//if(val[i] & b)
								if(!(val[i] & b))
									port_info[kBitsPlayValues[i][j][0+3]]->state->digital &= ~kBitsPlayValues[i][j][1+3];
						...
						for(j=0;j<29;j++)
						{
							if(kMahjongPlayValues[i][j][0]!=-1)
							{
								if(kMahjongPlayValues[i][j][2] != IP_ACTIVE_LOW) {
									...
									if(chval[KAILLERA_VAL_LEN + Kaillera_analog_port*KAILLERA_ANALOG_LEN + i*Kaillera_Val_Len] != j+1)
										port_info[kMahjongPlayValues[i][j][0]]->state->digital |= kMahjongPlayValues[i][j][1];
									else
										port_info[kMahjongPlayValues[i][j][0]]->state->digital &= ~kMahjongPlayValues[i][j][1];
								} else {
									//if(chval[KAILLERA_VAL_LEN + Kaillera_analog_port*KAILLERA_ANALOG_LEN + i*Kaillera_Val_Len] == j+1)
	...
	else
	{
		if (kFirstMsg)
		{
			const char *stemp = options_get_string(&machine->options(), "auto_record_name");
			kFirstMsg=0;
	...
	if (has_playback_sub_file(machine))
	{
		...
		{
			readword(handle_playback_sub_file(machine), (UINT16 *)&perform_ui);
			readword(handle_playback_sub_file(machine), (UINT16 *)&perform_ui_dat );
			readword(handle_playback_sub_file(machine), ((UINT16 *)&perform_ui_dat)+1 );
			readword(handle_playback_sub_file(machine), (UINT16 *)&perform_ui_count);
	...
	if (has_record_sub_file(machine))
	{
		...
		{
			int i;
			writeword(handle_record_sub_file(machine), (short)(perform_ui_count&0xffff));
			writeword(handle_record_sub_file(machine), (short)(perform_ui&0xffff));
			writeword(handle_record_sub_file(machine), (short)(perform_ui_dat&0xffff) );
			writeword(handle_record_sub_file(machine), (short)((perform_ui_dat>>16)&0xffff));
			perform_ui_count = 0;
...
#define IS_AUTOKEY(field)	((field->state->autofire & AUTOFIRE_ON) \
	...
	int pressed = input_seq_pressed(machine, input_field_seq(field, SEQ_TYPE_STANDARD));
...
#ifdef MAMEUIPLUSPLUS
		if (options_get_bool(&machine->options(), "disp_autofire_status"))
		{
			char tmp[256];
			
			sprintf(tmp, _("P%d %s"), field->player + 1, (field->state->toggle) ? _("Autofire On") : _("Autofire Off"));
			popmessage(tmp);
		}
#endif /* MAMEUIPLUSPLUS */
	}
	...
	if (field->type >= IPT_BUTTON1 && field->type < IPT_BUTTON1 + MAX_NORMAL_BUTTONS)
	{
		...
		for (custom = 0; custom < MAX_CUSTOM_BUTTONS; custom++)
			if (custom_button[field->player][custom] & button_mask)
			{
				...
				if (input_seq_pressed(machine, input_field_seq(custom_info, SEQ_TYPE_STANDARD)))
				{
	...
	if (normal_buttons > 0)
	{
		...
		for (port = machine->m_portlist.first(); port != NULL; port = port->next())
		{
			...
			for (field = port->fieldlist; field != NULL; field = field->next)
			{
				...
				if (field->player == player && input_seq_pressed(machine, input_field_seq(field, SEQ_TYPE_STANDARD)))
				{
		...
		for (i = 0; i < MAX_CUSTOM_BUTTONS; i++)
			if (custom_button[0][i] != 0)
			{
				...
				if (input_seq_pressed(machine, input_field_seq(custom_field, SEQ_TYPE_STANDARD)))
...
skip_comment:
	...
	if (caption_timer)
	{
		ui_draw_text_box(container, caption_text, JUSTIFY_LEFT, 0.5, 1.0, UI_BACKGROUND_COLOR);
		caption_timer--;
	}
}
#endif /* INP_CAPTION */
