...
static void menu_main_populate(running_machine &machine, ui_menu *menu, void *state)
{
	...
	menu->item_append(_("Input (general)"), NULL, 0, (void *)menu_input_groups);
...
#ifdef USE_AUTOFIRE
	menu->item_append(_("Autofire Setting"), NULL, 0, (void *)menu_autofire);
#endif /* USE_AUTOFIRE */
#ifdef USE_CUSTOM_BUTTON
	menu->item_append(_("Custom Buttons"), NULL, 0, (void *)menu_custom_button);
#endif /* USE_CUSTOM_BUTTON */

#ifdef KAILLERA
	if (!kPlay || ( kPlay && KailleraStartOption.player == 1))
#endif /* KAILLERA */
	/* add optional input-related menus */
	if (has_dips)
		menu->item_append(_("Dip Switches"), NULL, 0, (void *)menu_settings_dip_switches);
	if (has_configs)
		menu->item_append(_("Driver Configuration"), NULL, 0, (void *)menu_settings_driver_config);
	if (has_analog)
		menu->item_append(_("Analog Controls"), NULL, 0, (void *)menu_analog);

	/* add bookkeeping menu */
	menu->item_append(_("Bookkeeping Info"), NULL, 0, (void *)menu_bookkeeping);
	...
	{
		/* add image info menu */
		menu->item_append(_("Image Information"), NULL, 0, (void*)ui_image_menu_image_info);

		/* add file manager menu */
		menu->item_append(_("File Manager"), NULL, 0, (void*)ui_image_menu_file_manager);

		/* add tape control menu */
		if (machine.devicelist().first(CASSETTE))
			menu->item_append(_("Tape Control"), NULL, 0, (void*)ui_mess_menu_tape_control);

		/* add bitbanger control menu */
		if (machine.devicelist().first(BITBANGER))
			menu->item_append(_("Bitbanger Control"), NULL, 0, (void*)ui_mess_menu_bitbanger_control);
	}
	...
	{
		/* add image info menu */
		menu->item_append(_("Slot Devices"), NULL, 0, (void*)ui_menu_slot_devices);
	}

	/* add keyboard mode menu */
	if (input_machine_has_keyboard(machine) && inputx_can_post(machine))
		menu->item_append(_("Keyboard Mode"), NULL, 0, (void *)ui_menu_keyboard_mode);

#ifdef KAILLERA
	if (!kPlay)
#endif /* KAILLERA */
	/* add sliders menu */
	menu->item_append(_("Slider Controls"), NULL, 0, (void *)menu_sliders);

	/* add video options menu */
	menu->item_append(_("Video Options"), NULL, 0, (machine.render().target_by_index(1) != NULL) ? (void *)menu_video_targets : (void *)menu_video_options);
	...
	menu->item_append(_("Image Enhancement"), NULL, 0, (void *)menu_scale_effect);
#endif /* USE_SCALE_EFFECTS */

	/* add crosshair options menu */
	if (crosshair_get_usage(machine))
		menu->item_append(_("Crosshair Options"), NULL, 0, (void *)menu_crosshair);
	...
	if (machine.options().cheat() && machine.cheat().first() != NULL)
		menu->item_append(_("Cheat"), NULL, 0, (void *)menu_cheat);
	...
	if (machine.config().m_memcard_handler != NULL)
		menu->item_append(_("Memory Card"), NULL, 0, (void *)menu_memory_card);

#ifdef CMD_LIST
	/* add command list menu */
	menu->item_append(_("Show Command List"), NULL, 0, (void *)menu_command);
#endif /* CMD_LIST */
...
static void menu_input_groups_populate(running_machine &machine, ui_menu *menu, void *state)
{
	...
	/* build up the menu */
	menu->item_append(_("User Interface"), NULL, 0, (void *)(IPG_UI + 1));
	...
	menu->item_append(_("Other Controls"), NULL, 0, (void *)(FPTR)(IPG_OTHER + 1));
#ifdef KAILLERA
	menu->item_append(_("Special Controls"), NULL, 0, (void *)(FPTR)(IPG_SPECIAL + 1));
#endif /* KAILLERA */
}
...
static void menu_input_populate_and_sort(running_machine &machine, ui_menu *menu, input_item_data *itemlist, input_menu_state *menustate)
{
	...
	for (curitem = 0; curitem < numitems; curitem++)
	{
		...
		/* add the item */
		menu->item_append(_(text), _(subtext), flags, item);
	}
}
	...
	for (port = machine.m_portlist.first(); port != NULL; port = port->next())
		for (field = port->fieldlist().first(); field != NULL; field = field->next())
			if (field->type == type && input_condition_true(machine, &field->condition, port->owner()))
			{
				...
				/* add the menu item */
				menu->item_append(_(input_field_name(field)), _(input_field_setting_name(field)), flags, (void *)field);
...
static void menu_command(running_machine &machine, ui_menu *menu, void *parameter, void *state)
{
	...
	if (!menu->populated())
	{
		...
		if (total)
		{
			for (menu_items = 0; menu_items < total; menu_items++)
				menu->item_append(item[menu_items], NULL, 0, (void *)menu_items);
		}
	}
	...
	if (event != NULL && event->iptkey == IPT_UI_SELECT)
		ui_menu::stack_push(ui_menu_alloc(machine, menu->container, menu_command_content, event->itemref));
}
...
//		menu->item_append(commandbuf, NULL, MENU_FLAG_MULTILINE, NULL);
...
static void menu_cheat_populate(running_machine &machine, ui_menu *menu)
{
	...
	menu->item_append(_("Reset All"), NULL, 0, (void *)1);

	/* add a reload all cheats option */
	menu->item_append(_("Reload All"), NULL, 0, (void *)2);
}
...
static void menu_memory_card_populate(running_machine &machine, ui_menu *menu, int cardnum)
{
	...
	if (cardnum < 1000)
		flags |= MENU_FLAG_RIGHT_ARROW;
	menu->item_append(_("Card Number:"), tempstring, flags, (void *)MEMCARD_ITEM_SELECT);

	/* add the remaining items */
	menu->item_append(_("Load Selected Card"), NULL, 0, (void *)MEMCARD_ITEM_LOAD);
	if (memcard_present(machine) != -1)
		menu->item_append(_("Eject Current Card"), NULL, 0, (void *)MEMCARD_ITEM_EJECT);
	menu->item_append(_("Create New Card"), NULL, 0, (void *)MEMCARD_ITEM_CREATE);
}
...
static void menu_video_options_populate(running_machine &machine, ui_menu *menu, render_target *target)
{
	...
	menu->item_append(_("Rotate"), subtext, MENU_FLAG_LEFT_ARROW | MENU_FLAG_RIGHT_ARROW, (void *)VIDEO_ITEM_ROTATE);

	/* backdrop item */
	enabled = target->backdrops_enabled();
	menu->item_append(_("Backdrops"), enabled ? _("Enabled") : _("Disabled"), enabled ? MENU_FLAG_LEFT_ARROW : MENU_FLAG_RIGHT_ARROW, (void *)VIDEO_ITEM_BACKDROPS);

	/* overlay item */
	enabled = target->overlays_enabled();
	menu->item_append(_("Overlays"), enabled ? _("Enabled") : _("Disabled"), enabled ? MENU_FLAG_LEFT_ARROW : MENU_FLAG_RIGHT_ARROW, (void *)VIDEO_ITEM_OVERLAYS);

	/* bezel item */
	enabled = target->bezels_enabled();
	menu->item_append(_("Bezels"), enabled ? _("Enabled") : _("Disabled"), enabled ? MENU_FLAG_LEFT_ARROW : MENU_FLAG_RIGHT_ARROW, (void *)VIDEO_ITEM_BEZELS);

	/* cpanel item */
	enabled = target->cpanels_enabled();
	menu->item_append(_("CPanels"), enabled ? _("Enabled") : _("Disabled"), enabled ? MENU_FLAG_LEFT_ARROW : MENU_FLAG_RIGHT_ARROW, (void *)VIDEO_ITEM_CPANELS);

	/* marquee item */
	enabled = target->marquees_enabled();
	menu->item_append(_("Marquees"), enabled ? _("Enabled") : _("Disabled"), enabled ? MENU_FLAG_LEFT_ARROW : MENU_FLAG_RIGHT_ARROW, (void *)VIDEO_ITEM_MARQUEES);

	/* cropping */
	enabled = target->zoom_to_screen();
	menu->item_append(_("View"), enabled ? _("Cropped") : _("Full"), enabled ? MENU_FLAG_RIGHT_ARROW : MENU_FLAG_LEFT_ARROW, (void *)VIDEO_ITEM_ZOOM);
}
...
static void menu_crosshair_populate(running_machine &machine, ui_menu *menu)
{
	...
	for (player = 0; player < MAX_PLAYERS; player++)
	{
		...
		if (settings.used)
		{
			...
			menu->item_append(temp_text, _(vis_text[settings.mode]), flags, data);
			...
			menu->item_append(temp_text, using_default ? _("DEFAULT") : settings.name, flags, data);
		}
	}
	if (use_auto)
	{
		...
		menu->item_append(_("Visible Delay"), temp_text, flags, data);
	}
//  else
...
{
	int scaler;
	menu->item_append(_("None"), NULL, 0, (void *)SCALE_ITEM_NONE);
	...
	for (scaler = 1; ; scaler++)
	{
		...
		menu->item_append(desc, NULL, 0, (void *)(SCALE_ITEM_NONE + scaler));
	}
...
//				entry[menu_items] = field;
				...
				menu->item_append(_(input_field_name(field)), subtext, MENU_FLAG_LEFT_ARROW | MENU_FLAG_RIGHT_ARROW, (void *)field);
			}
		}
	...
	for (i = 0; i < players; i++)
	{
		...
		/* append a menu item */
		menu->item_append(text, subtext, MENU_FLAG_LEFT_ARROW | MENU_FLAG_RIGHT_ARROW, (void *)(i + AUTOFIRE_ITEM_P1_DELAY));
	}
}
	...
	for (port = machine.m_portlist.first(); port != NULL; port = port->next())
		for (field = port->first_field(); field != NULL; field = field->next())
		{
			...
			if (name != NULL && type >= IPT_CUSTOM1 && type < IPT_CUSTOM1 + MAX_CUSTOM_BUTTONS)
			{
				...
				convert_command_glyph(commandbuf, ARRAY_LENGTH(commandbuf));
				menu->item_append(_(name), commandbuf, 0, (void *)(FPTR)&custom_button[player][type]);
...
static void menu_select_game_populate(running_machine &machine, ui_menu *menu, select_game_state *menustate)
{
	...
	for (curitem = 0; curitem < matchcount; curitem++)
	{
		...
		if (curmatch != -1)
		{
			...
			menu->item_append(drivlist->driver(curmatch).name, _LST(drivlist->driver(curmatch).description), (cloneof == -1) ? 0 : MENU_FLAG_INVERT, (void *)&drivlist->driver(curmatch));
		}
	}
	...
	{
		menu->item_append(MENU_SEPARATOR_ITEM, NULL, 0, NULL);
		menu->item_append(_("Configure General Inputs"), NULL, 0, (void *)1);
	}
