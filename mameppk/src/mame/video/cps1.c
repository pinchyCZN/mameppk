...
static int region_switch_offset3;
static int region_switch_data3;
static int region_switch_mask3;
static int region_switch_count3;
static int region_switch_counter3;

READ16_HANDLER( cps2_region_16_r )
{
	UINT16 mem = cps2_ram[region_switch_offset >> 1];
...
{
	UINT16 mem = cps2_ram[region_switch_offset2 >> 1];
...
READ16_HANDLER( cps2_region_16_r3 )
{
	cps_state *state = space->machine->driver_data<cps_state>();
	UINT16 mem = state->gfxram[region_switch_offset3 >> 1];

	if (region_switch_counter3 < region_switch_count3)
	{
		region_switch_counter3++;
		return mem;
	}
	return (mem & region_switch_mask3) | region_switch_data3;
}
...
{                                                         \
    ...
    memory_install_read16_handler(cputag_get_address_space(machine, "maincpu", ADDRESS_SPACE_PROGRAM), OFFSET, OFFSET+1, 0, 0, cps2_region_16_r); \
}
...
{                                                          \
    ...
    memory_install_read16_handler(cputag_get_address_space(machine, "maincpu", ADDRESS_SPACE_PROGRAM), OFFSET, OFFSET+1, 0, 0, cps2_region_16_r2); \
}

#define CPS2_REGION_16_R3(NAME, OFFSET, DATA, COUNT, MASK) \
if (!strcmp(machine->gamedrv->name, NAME))                 \
{                                                          \
    region_switch_offset3  = OFFSET & 0x3ffff;             \
    region_switch_data3    = DATA;                         \
    region_switch_mask3    = MASK;                         \
    region_switch_count3   = COUNT;                        \
    if (region_switch_mask3 == MASK_LOW)                   \
        region_switch_data3 <<= 8;                         \
    memory_install_read16_handler(cputag_get_address_space(machine, "maincpu", ADDRESS_SPACE_PROGRAM), OFFSET, OFFSET+1, 0, 0, cps2_region_16_r3); \
}
...
#ifdef KAILLERA
	CPS2_REGION_16_R ("xmvsfregion4p",  0xff5478, 0, 4, MASK_HIGH)
#endif /* KAILLERA */
#endif /* MAMEUIPLUSPLUS */

#ifdef KAILLERA
    cps2_nvram_type = NV_NORMAL;
	if (!strcmp(machine->gamedrv->name, "avsp3p"))      cps2_nvram_type = NV_avsp3p;
	if (!strcmp(machine->gamedrv->name, "avspa3p"))     cps2_nvram_type = NV_avspa3p;
	if (!strcmp(machine->gamedrv->name, "avspu3p"))     cps2_nvram_type = NV_avspu3p;
	if (!strcmp(machine->gamedrv->name, "avspj3p"))     cps2_nvram_type = NV_avspj3p;
	if (!strcmp(machine->gamedrv->name, "batcir4p"))    cps2_nvram_type = NV_batcir4p;
	if (!strcmp(machine->gamedrv->name, "batcirj4p"))   cps2_nvram_type = NV_batcirj4p;
	if (!strcmp(machine->gamedrv->name, "ddsom4p"))     cps2_nvram_type = NV_ddsom4p;
	if (!strcmp(machine->gamedrv->name, "ddsomr2_4p"))  cps2_nvram_type = NV_ddsomr2_4p;
	if (!strcmp(machine->gamedrv->name, "ddsomu4p"))    cps2_nvram_type = NV_ddsomu4p;
	if (!strcmp(machine->gamedrv->name, "ddsomur1_4p")) cps2_nvram_type = NV_ddsomur1_4p;
	if (!strcmp(machine->gamedrv->name, "ddsomj4p"))    cps2_nvram_type = NV_ddsomj4p;
	if (!strcmp(machine->gamedrv->name, "ddsomjr1_4p")) cps2_nvram_type = NV_ddsomjr1_4p;
	if (!strcmp(machine->gamedrv->name, "ddsoma4p"))    cps2_nvram_type = NV_ddsoma4p;
	if (!strcmp(machine->gamedrv->name, "sfa3p"))       cps2_nvram_type = NV_sfa3p;
	if (!strcmp(machine->gamedrv->name, "sfa3up"))      cps2_nvram_type = NV_sfa3up;
	if (!strcmp(machine->gamedrv->name, "sfa3ur1p"))    cps2_nvram_type = NV_sfa3ur1p;
	if (!strcmp(machine->gamedrv->name, "sfz3jp"))      cps2_nvram_type = NV_sfz3jp;
	if (!strcmp(machine->gamedrv->name, "sfz3jr1p"))    cps2_nvram_type = NV_sfz3jr1p;
	if (!strcmp(machine->gamedrv->name, "sfz3jr2p"))    cps2_nvram_type = NV_sfz3jr2p;
	if (!strcmp(machine->gamedrv->name, "sfz3ar1p"))    cps2_nvram_type = NV_sfz3ar1p;
#ifdef JAPANESE
	if (!strcmp(machine->gamedrv->name, "hsf2nb"))      cps2_nvram_type = NV_hsf2anb;
	if (!strcmp(machine->gamedrv->name, "hsf2d"))       cps2_nvram_type = NV_hsf2anb;
#endif /* JAPANESE */

	SelectMemoryHack( machine->gamedrv->name );
#endif /* KAILLERA */
}
