...
static MACHINE_START( cps3 )
{
	...
	{
		default_dip = rom[0x1fed8/4] & 0x0000000f;
	}
	else
	{
		default_dip = rom[0x1fec8/4] & 0x0000000f;
	}
	/* switch version */
	if(rom[0x1fed4/4] == 0x575a442d)
	{
		default_ver_dip = rom[0x1fed8/4] & 0x000000f0;
	}
	else if (rom[0x1fec4/4] == 0x4a4a4b2d || rom[0x1fec4/4] == 0x4a4a4d2d)
	{
		default_ver_dip = rom[0x1fec8/4] & 0x000000f0;
	}
#endif /* MAMEUIPLUSPLUS */
...
static MACHINE_RESET( cps3 )
{
...

	{
		...
		{
#ifdef MAMEUIPLUSPLUS
			if (!reg_dip) reg_dip = default_dip;
#else
...
#ifdef MAMEUIPLUSPLUS
			if (!ver_dip) ver_dip = default_ver_dip;
#else
...
#ifdef MAMEUIPLUSPLUS
	if (!read_or_write && !mame_stricmp(machine->gamedrv->name,"jojobap"))
	{
		UINT8 eeprom[]={
			0x00,0x00,0x01,0x00,0x01,0x01,0x00,0x00,0x00,0x01,0x01,0x01,0x00,0x00,0x00,0x00,
			0x00,0x00,0x00,0x00,0x01,0x01,0x00,0x00,0x00,0x00,0x01,0x01,0x00,0x00,0x01,0x01,
			0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x4D,0x4A,0x4A,
			0x00,0x00,0x01,0x00,0x01,0x01,0x00,0x00,0x00,0x01,0x01,0x01,0x00,0x00,0x00,0x00,
			0x00,0x00,0x00,0x00,0x01,0x01,0x00,0x00,0x00,0x00,0x01,0x01,0x00,0x00,0x01,0x01,
			0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x4D,0x4A,0x4A
		};
		memset(cps3_eeprom,0,0x400);
		memcpy(cps3_eeprom, eeprom, sizeof(eeprom));
	}
#endif /* MAMEUIPLUSPLUS */
...
#ifdef MAMEUIPLUSPLUS
GAME_HACK( 1999, jojobap, jojoban,   cps3, jojo, jojoba, ROT0,   "Capcom", "JoJo's Bizarre Adventure: Heritage for the Future / JoJo no Kimyouna Bouken: Miraie no Isan (Japan, 990913 / Ex Chars)", GAME_IMPERFECT_GRAPHICS )
#endif /* MAMEUIPLUSPLUS */
