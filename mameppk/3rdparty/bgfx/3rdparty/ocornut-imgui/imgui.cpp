...
#pragma clang diagnostic ignored "-Wunused-parameter"         // warning: unused parameter exxxxf
#elif defined(__GNUC__)
#pragma GCC diagnostic ignored "-Wunused-function"          // warning: 'xxxx' defined but not used
#pragma GCC diagnostic ignored "-Wunused-parameter"         // warning: unused parameter exxxxf
#pragma GCC diagnostic ignored "-Wtype-limits"              // warning: comparison is always true due to limited range of data type
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"      // warning: exxxxf may be used uninitialized in this function
#endif
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    const ImGuiAabb bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + style.FramePadding.x*2 + (label_size.x > 0.0f ? style.ItemInnerSpacing.x : 0.0f), 0.0f) + label_size);
    ItemSize(bb);
...
bool ImGui::SmallButton(const char* label)
{
    ...
    const ImGuiAabb bb(window->DC.CursorPos, window->DC.CursorPos + label_size + ImVec2(style.FramePadding.x*2,0));
    ItemSize(bb);
    if (!ItemAdd(bb, &id))
        return false;
